...
function updateInitialName() {
  socket.on("updateInitialPlayerName", data => {
    const mydata = JSON.parse(data)
    mydata.forEach((data) => {
      if (data.socket == socketid) {
        $("#mainName").html(`${data.name}`)
        mydata.forEach((player) => {
          ...
          position = getPlayerPosition(player, data);
          if (position != "unavailable") {
            if (position != "a5") {
              positionString = "#" + position + " .playerName";
              $(positionString).html(`${player.name}`);
            }
          }
          else {
